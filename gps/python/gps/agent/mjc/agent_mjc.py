...
class AgentMuJoCo(Agent):
    ...
    def sample(self, policy, condition, verbose=True, save=True, noisy=True):
        ...
        mj_X = self._hyperparams['x0'][condition]     #initial state in mj_world, uncondition-specific
    ...
    def _init_sample(self, condition, feature_fn=None):
        ...
        data = self._world[condition].get_data()          #get data from mj_world, condition-specific
        sample.set(JOINT_ANGLES, data['qpos'].flatten(), t=0)    #Set _data in sample class
    ...
    def _set_sample(self, sample, mj_X, t, condition, feature_fn=None):
        ...
        """
        sample.set(JOINT_ANGLES, np.array(mj_X[self._joint_idx]), t=t+1)   #Set _data in sample class
